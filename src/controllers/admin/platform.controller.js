...
const createPlatform = async (req, res )=>{
    ...
    // let untilted_id =req.companyData.business_id;
    let untilted_id = 1;
    //validate input data
    if (!platform_name) {
    ...
    let trimPlatformName = platform_name.trim();
    //check if the platform with the provided  platform name exists 
    const platformQuery = "SELECT * FROM  platform  WHERE TRIM(LOWER(platform_name)) = $1";
    const platformResult = await pool.query(platformQuery, [trimPlatformName.toLowerCase()]);
    ...
    try {
        ...
        const insertQuery = "INSERT INTO platform (platform_name,api_key, description) VALUES ($1, $2, $3)";
...
const updatePlatform = async (req, res)=>{
    const platform_id = parseInt(req.params.id);
    const platform_name = req.body.platform_name?req.body.platform_name.trim():null;
    ...
    const existingPlatformQuery = "SELECT * FROM platform  WHERE  LOWER(platform_name) = $1 AND platform_id != $2";
    const existingPlatformResult = await pool.query(existingPlatformQuery, [platform_name.toLowerCase(), platform_id]);
...
const onStatusChange = async (req, res) =>{
    ...
    try {
        ...
        await pool.query(updateQuery, [platform_id, status]);
